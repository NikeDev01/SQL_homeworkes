...
select s.c.FirstName, c.LastName, o.OrderDate
FROM Customers c
join  orgers o on c.CustomerID = o.CustomerID
...
select  d.DepartmentName, , count(e.EmployeeID) as EmployeeID
from Departments d
...
--Bir xil rahbarga bo‘ysunadigan xodimlar juftligi
...
--2022-yilda berilgan buyurtmalar va mijoz ismlari
...
---Sales' bo‘limida ishlaydigan va maoshi 60000 dan yuqori bo‘lgan xodimlar
...
--. Faqat to‘lov bilan bog‘langan buyurtmalar
...
--. O‘z bo‘limining o‘rtacha maoshidan ko‘proq oladigan xodimlar
...
---Hech qaysi toifaga (category) tegishli bo‘lmagan mahsulotlar
...
ELECT p.ProductID, p.ProductName
FROM Products p
...
FROM Employees e1
JOIN Employees e2 ON e1.ManagerID = e2.ManagerID AND e1.EmployeeID < e2.EmployeeID
WHERE e1.Salary > 60000 AND e1.ManagerID IS NOT NULL;


--Bo‘lim nomi 'M' harfi bilan boshlanadigan bo‘limdagi xodimlar
...
--Sotuv summasi 500 dan yuqori bo‘lgan sotuvlar va mahsulot nomlari
...
-- to‘lov ma’lumotlari yo‘q bo‘lgan buyurtmalar
...
--'Electronics' yoki 'Furniture' toifasiga tegishli mahsulotlar
